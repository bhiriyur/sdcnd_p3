...
def process_image(path, steering_angle, augment, shape=(100, 100),data_dir=None):
    """Process and augment an image."""
    if data_dir is not None:
        image = load_img(data_dir+path, target_size=shape)
    else:
...
def generator(batch_size, X, y):
    ...
    while 1:
        ...
        for i in range(batch_size):
            ...
            sa = y[sample_index]
            image, sa = process_image(X[sample_index], sa, augment=True,data_dir='data/')
...
def train():
    ...
    net.fit_generator(generator(256, X, y), samples_per_epoch=20224, nb_epoch=2)
